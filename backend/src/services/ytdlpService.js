...
function normalizeFormats(rawFormats, platform) {
  ...
  for (const f of sortedVideo) {
    const quality = f.height
      ? `${f.height}p`
      : f.format_note || f.format_id || 'unknown';
  ...
  if (
    audioAdded.size === 0 &&
    videoFormats.some((f) => f.acodec && f.acodec !== 'none')
  ) {
    normalized.push({
      formatId: 'bestaudio',
      quality: 'Audio Only',
...
async function extractMetadata(url) {
  ...
  try {
    ...
    if (msg.includes('removed') || msg.includes('deleted')) {
...
async function getDownloadUrl(url, formatId) {
  ...
  if (cached && formatId && formatId !== 'bestaudio') {
    const cachedFormat = cached.formats.find(
    ...
    if (cachedFormat && cachedFormat.url) {
      ...
      return {
        directUrl: cachedFormat.url,
        ext: cachedFormat.ext,
        title: cached.title,
      };
    }
  }
